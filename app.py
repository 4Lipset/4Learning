...
STANDARDS = {
    "Common Core": {
        ...
        "Humanities": [
            "CCSS.ELA-LITERACY.RH.11-12.1: Cite specific textual evidence...",
            "CCSS.ELA-LITERACY.RH.11-12.9: Integrate information from diverse sources..."
        ],
        "Art": ["CCSS.ART.11-12.CR.1"],
        "Technology": ["ISTE 6a", "ISTE 6b"],
        "Music": ["MU:Cr1.1.HS1"],
        "Entrepreneurship": ["ENT.C1.1"]
    },
    "California": {
        "Science": ["CA NGSS HS-PS1-4", "CA NGSS HS-LS2-7"],
        "Language Arts": ["CA CCSS.ELA-LITERACY.W.11-12.1", "CA CCSS.ELA-LITERACY.SL.11-12.5"],
        "Humanities": ["CA HSS 11.2.1", "CA HSS 11.11.5"],
        "Art": ["CA VAPA PROF.CR1.1"],
        "Technology": ["CA CTE.IT.C1.1"],
        "Music": ["CA VAPA PROF.MU:Cr1.1.HS"],
        "Entrepreneurship": ["CA CTE.BE.A1.1"]
    },
    "New York": {
        "Science": ["NYSED P-12.SCI.9-12.PS3.3"],
        "Language Arts": ["NYS Next Gen ELA 11-12W6"],
        "Humanities": ["NYS SS 11.4a"],
        "Art": ["NYSED VA.Cr1.1.I"],
        "Technology": ["NYSED TECH.K12.6"],
        "Music": ["NYSED MU.Cr1.1.HS"],
        "Entrepreneurship": ["NYS CDOS.3a" ]
    },
    "Minnesota": {
        "Science": ["MN Sci 9C.2.1.2"],
        "Language Arts": ["MN ELA 11.7.2.2"],
        "Humanities": ["MN SS 9.4.1.2.1"],
        "Art": ["MN ART 9.1.1.5.1"],
        "Technology": ["MN TECH 9.2.1.1"],
        "Music": ["MN MU:Cr1.1.HS1"],
        "Entrepreneurship": ["MN Career E1.2.1" ]
    },
    "Texas": {
        "Science": ["TEKS SCI 112.39.b.5"],
        "Language Arts": ["TEKS ELA 110.36.b.10"],
        "Humanities": ["TEKS SS 113.44.c.6"],
        "Art": ["TEKS ART 117.52.c.1"],
        "Technology": ["TEKS TECH 126.42.c.5"],
        "Music": ["TEKS MU 117.64.c.2"],
        "Entrepreneurship": ["TEKS CTE 130.347.c.2"]
    }
}
...
# Utility to format standard links
...
# Streamlit form and output handling
with st.form("unit_form"):
    ...
    if state and subject:
        available = STANDARDS.get(state, {}).get(subject, [])
        if available:
            st.markdown("**Available standards for this selection:**")
            for std in available:
                st.code(std, language='text')
                if state == "California":
                    url = f"https://www.cde.ca.gov/search/?q={std.replace(' ', '+')}"
                elif state == "Texas":
                    url = f"https://tea.texas.gov/search-results?q={std.replace(' ', '+')}"
                elif state == "New York":
                    url = f"https://www.nysed.gov/search/node/{std.replace(' ', '%20')}"
                elif state == "Minnesota":
                    url = f"https://education.mn.gov/mde/search/index.htm?q={std.replace(' ', '+')}"
                else:
                    url = f"https://www.google.com/search?q={std.replace(' ', '+')}"
                st.markdown(f"📎 [Learn more about this standard]({url})")
...
if submitted:
    ...
    doc.save(doc_buffer)
    doc_buffer.seek(0)
    st.download_button("📥 Download Unit Plan (DOCX)", data=doc_buffer, file_name=f"{title.replace(' ', '_')}.docx")
    ...
    pdf_buffer.seek(0)
    st.download_button("🖨️ Download Unit Plan (PDF)", data=pdf_buffer, file_name=f"{title.replace(' ', '_')}.pdf")
